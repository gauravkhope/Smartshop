...
function imageFor(query: string, idx: number) {
...
function makeName(brand: string, sub: string, variant: string | number) {
...
const CATS: {
  ...
  subs: {
    name: string;
    brands: string[];
    titlePatterns: string[]; // pattern words to create names
    imageQuery?: string;
  }[];
}[] = [
  {
  ...
  for (const sub of cat.subs) {
    ...
    for (let i = 0; i < countPerSub; i++) {
      ...
      const basePriceCategory = (() => {
        ...
        if (main.includes("electronics")) return rand(5000, 150000);
        if (main.includes("fashion") || main.includes("apparel")) return rand(499, 14999);
        if (main.includes("home")) return rand(999, 79999);
        if (main.includes("beauty") || main.includes("health")) return rand(199, 14999);
        if (main.includes("food")) return rand(49, 1999);
        if (main.includes("baby")) return rand(199, 7999);
        if (main.includes("sports")) return rand(499, 49999);
        if (main.includes("automotive")) return rand(299, 49999);
        if (main.includes("books")) return rand(99, 1999);
        if (main.includes("pet")) return rand(199, 9999);
        if (main.includes("office")) return rand(99, 19999);
        if (main.includes("digital")) return rand(199, 49999);
        if (main.includes("gifts")) return rand(99, 9999);
