  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="relative w-40 h-40">
            <svg className="transform -rotate-90 w-40 h-40">
              ...
              <circle cx="80" cy="80" r="70" stroke="url(#gradient)" strokeWidth="12" fill="none" strokeDasharray={`${(averageRating / 5) * 439.6} 439.6`} strokeLinecap="round" className="transition-all duration-1000" />
              <defs>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <div className="flex items-center gap-1 mt-1">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={16} className={i < Math.floor(averageRating) ? "fill-orange-400 text-orange-400" : "text-gray-300"} />
                ))}
              </div>
            </div>
          </div>
          ...
          {ratingDistribution.map((rating) => (
            <div key={rating.stars} className="flex items-center gap-3">
              ...
              <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                <div className="bg-gradient-to-r from-orange-400 to-pink-400 h-full rounded-full transition-all duration-500" style={{ width: `${rating.percentage}%` }} />
              </div>
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 w-16 text-right">{rating.percentage}%</span>
              <span className="text-xs text-gray-500 dark:text-gray-400 w-16 text-right">({rating.count})</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-5 gap-3">
          {displayedImages.map((img) => (
            <div key={img.id} className="relative aspect-square rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-700 hover:border-orange-500 transition cursor-pointer group">
              <Image src={img.url} alt={`Customer photo ${img.id}`} fill className="object-cover group-hover:scale-110 transition" unoptimized />
            </div>
          ))}
          {!showAllImages && customerImages.length > 5 && (
            <button onClick={() => setShowAllImages(true)} className="relative aspect-square rounded-xl overflow-hidden border-2 border-dashed border-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition flex flex-col items-center justify-center gap-2 group">
              <Plus size={32} className="text-orange-500 group-hover:scale-110 transition" />
              <span className="text-sm font-semibold text-orange-500">+{customerImages.length - 5}</span>
            </button>
          )}
        </div>
        {showAllImages && (
          <button onClick={() => setShowAllImages(false)} className="mt-4 text-orange-500 font-semibold hover:underline">Show Less</button>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-3 gap-4">
          {customerVideos.map((video) => (
            <div key={video.id} className="relative aspect-video rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-700 hover:border-orange-500 transition cursor-pointer group">
              <Image src={video.thumbnail} alt={`Customer video ${video.id}`} fill className="object-cover" unoptimized />
              ...
              <div className="absolute bottom-2 right-2 bg-black/70 px-2 py-1 rounded text-white text-xs font-semibold">{video.duration}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {[
          { name: "Rahul Sharma", rating: 5, verified: true, comment: "Excellent product! Totally worth the price. The quality is amazing and delivery was super fast. I've been using it for 2 weeks now and it exceeded all my expectations.", date: "2 days ago", helpful: 124, images: 3 },
          { name: "Priya Patel", rating: 4, verified: true, comment: "Good quality, fast delivery. Very satisfied with my purchase! The only reason I'm giving 4 stars instead of 5 is because the packaging could be better.", date: "1 week ago", helpful: 87, images: 0 },
          { name: "Amit Kumar", rating: 5, verified: true, comment: "Amazing! Exceeded my expectations. Highly recommended! This is my second purchase and the quality remains consistent.", date: "2 weeks ago", helpful: 56, images: 2 },
          { name: "Sneha Reddy", rating: 3, verified: false, comment: "Average product. Expected better quality for this price point. It works fine but not as premium as described.", date: "3 weeks ago", helpful: 23, images: 1 },
        ].map((review, idx) => (
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-gray-900 dark:text-white text-lg">{review.name}</span>
                    {review.verified && (
            ...
            {review.images > 0 && (
              <div className="flex gap-2 mb-4">
                {Array.from({ length: review.images }).map((_, i) => (
                  <div key={i} className="w-24 h-24 bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden border-2 border-gray-300 dark:border-gray-600 hover:border-orange-500 transition cursor-pointer">
                    <Image src={`https://picsum.photos/200/200?random=review${idx}${i}`} alt={`Review image ${i + 1}`} width={96} height={96} className="object-cover w-full h-full hover:scale-110 transition" unoptimized />
                  </div>
                ))}
              </div>
            )}
...
export default function ProductDetailsPage() {
  ...
  const productId = params ? params['productId'] : undefined;
  ...
  const [selectedViewIndex, setSelectedViewIndex] = useState(3);
  ...
  const getViewStyle = (viewIndex: number) => {
    const styles = [
      { scale: "scale-150", translate: "translate-y-4", crop: true },
      { scale: "scale-125", translate: "", crop: false },
      { scale: "scale-175", translate: "", crop: false },
      { scale: "scale-110", translate: "", crop: false },
    ];
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 shadow-xl z-50 transition-transform duration-300 border-b border-gray-200 dark:border-gray-700 ${
          showStickyBar ? "translate-y-0" : "-translate-y-full"
        }`}>
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Image src={product.image} alt={product.name} width={50} height={50} className="object-contain bg-white rounded-lg p-1" unoptimized />
            <div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 rounded-lg px-2 py-1">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition">
                <Minus size={16} />
              </button>
              <span className="font-bold w-8 text-center">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition">
                <Plus size={16} />
              </button>
            </div>
            <button
      ...
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-8 px-4">
        <div className="max-w-7xl mx-auto">
          {/* Back Button */}
          <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-orange-500 transition mb-6 group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden aspect-square">
                  ...
                  return (
                    <button
                      ...
                      className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all transform hover:scale-105 hover:shadow-lg ${
                        isSelected ? "border-orange-500 shadow-lg ring-4 ring-orange-300/50" : "border-gray-200 dark:border-gray-700 hover:border-orange-300"
                      }`}
                    >
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <div className="inline-block px-4 py-1 bg-gradient-to-r from-orange-100 to-pink-100 dark:from-orange-900/30 dark:to-pink-900/30 rounded-full">
                  <span className="text-sm font-semibold text-orange-600 dark:text-orange-400">{product.brand || "Brand"}</span>
                </div>
              ...
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white leading-tight">{product.name}</h1>

              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1 bg-green-500 text-white px-3 py-1 rounded-lg">
                  <span className="font-bold">{product.rating || 4.2}</span>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-pink-50 dark:from-gray-800 dark:to-gray-700 rounded-2xl p-6">
                <div className="flex items-baseline gap-4">
                  ...
                  <span className="text-2xl text-gray-400 line-through">{formatPrice(originalPrice)}</span>
                  <span className="text-green-600 font-bold text-xl">Save {formatPrice(originalPrice - finalPrice)}</span>
                </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={checkPincode} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition font-semibold">
                    Check
                  </button>
                </div>
              ...
              <div ref={addToCartRef}>
                <label className="block font-semibold text-gray-700 dark:text-gray-300 mb-3">Quantity:</label>
                <div className="flex items-center gap-3 bg-white dark:bg-gray-800 rounded-lg shadow-md px-2 py-1 w-fit">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition">
                    <Minus size={20} />
                  </button>
                  <span className="font-bold text-2xl w-16 text-center">{quantity}</span>
                  <button onClick={() => setQuantity(Math.min(product.stock || 50, quantity + 1))} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition">
                    <Plus size={20} />
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  >
                    <ShoppingCart size={24} />
                    {isInCart(product.id) ? `Add More (${quantity}) ✓` : `Add to Cart (${quantity})`}
                  </button>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    <span className="font-semibold">{isWishlisted ? "Wishlisted" : "Add to Wishlist"}</span>
                  </button>

                  <button onClick={handleShare} className="flex-1 flex items-center justify-center gap-2 p-4 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-xl shadow-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all hover:scale-105 border-2 border-gray-200 dark:border-gray-700">
                    <Share2 size={24} />
          ...
          <div className="mt-12">
              ...
              {(["overview", "specs"] as const).map((tab) => (
                <button
                  ...
                  className={`px-6 py-3 font-semibold capitalize transition-all relative ${
                    activeTab === tab ? "text-orange-500" : "text-gray-600 dark:text-gray-400 hover:text-orange-400"
                  }`}
                >
                  {tab}
                  {activeTab === tab && (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
              {activeTab === "overview" && (
                <div className="space-y-6">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 text-lg leading-relaxed">
                    {product.description || `Premium quality ${product.name} from ${product.brand}. Experience excellence with cutting-edge features and stunning design. Perfect blend of style and functionality for modern living. Crafted with precision and attention to detail, this product ensures durability and long-lasting performance.`}
                  </p>
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    {["Premium Quality Material", "Modern & Sleek Design", "Durable & Long-lasting", "Easy to Use & Maintain"].map((feature, idx) => (
              ...
              {activeTab === "specs" && (
                <div>
                  <h3 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Product Specifications</h3>
                  <div className="space-y-3">
                    {[
                      { label: "Brand", value: product.brand || "Brand" },
                      { label: "Model", value: product.name },
                      { label: "Category", value: product.category || "General" },
                      { label: "Warranty", value: "1 Year Manufacturer Warranty" },
                      { label: "Availability", value: `${product.stock || 50} in Stock`, highlight: true },
                      { label: "Country of Origin", value: "India" },
                    ].map((spec, idx) => (
                      <div key={idx} className="flex justify-between py-4 border-b border-gray-200 dark:border-gray-700 last:border-0">
                        <span className="font-semibold text-gray-700 dark:text-gray-300">{spec.label}</span>
                        <span className={spec.highlight ? "text-green-600 font-semibold" : "text-gray-600 dark:text-gray-400"}>{spec.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="mt-12 bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-4">
              {[
                { q: "What is the return policy?", a: "We offer a 7-day return policy from the date of delivery. Product must be unused and in original packaging." },
                { q: "Is Cash on Delivery available?", a: "Yes, COD is available for orders below ₹50,000. A small COD fee may apply." },
                { q: "How long will delivery take?", a: "Delivery typically takes 3-5 business days depending on your location." }
              ].map((faq, idx) => (
      ...
      {showImageModal && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 animate-fadeIn" onClick={() => setShowImageModal(false)}>
          <button onClick={() => setShowImageModal(false)} className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white p-3 rounded-full transition z-10">
            <X size={24} />
          </button>
              ...
              {[0, 1, 2, 3].map((viewIdx) => (
                <button
                  ...
                  className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${
                    selectedViewIndex === viewIdx ? "border-orange-500 scale-110" : "border-white/30 hover:border-white/60"
                  }`}
                >
