// frontend/data/homepageData.ts

import Fshionweek from "./images/Fshionweek.jpg";
import MobilesLaptop from "./images/MobilesLaptop.jpg";

const heroBanners = [
  { id: "h1", image: "https://t3.ftcdn.net/jpg/04/65/46/52/360_F_465465254_1pN9MGrA831idD6zIBL7q8rnZZpUCQTy.jpg", title: "Mega Electronics Sale", subtitle: "Up to 50% off selected items" },
  { id: "h2", image: Fshionweek, title: "Fashion Week", subtitle: "New arrivals — limited stock" },
  { id: "h3", image: "https://img.freepik.com/premium-vector/sale-home-appliances-banner-template-design_958026-92.jpg", title: "Home Appliances", subtitle: "Smart appliances, smart prices" },
  { id: "h4", image: MobilesLaptop, title: "Mobiles & Laptops", subtitle: "Top brands, best deals" },
  { id: "h5", image: "https://graphicsfamily.com/wp-content/uploads/edd/2024/11/Creative-E-Commerce-Shoes-Banner-Design-Free-PSD--scaled.jpg", title: "Footwear Sale", subtitle: "Step into style" },
];

const trending = [
  { id: "t1", name: "iPhone 17 pro max", brand: "Apple", price:"1,49,999", image: "https://media-ik.croma.com/prod/https://media.tatacroma.com/Croma%20Assets/Communication/Mobiles/Images/317417_0_7ISiBcc4Y.png?updatedAt=1757529273198" },
  { id: "t2", name: "Galaxy S25 ultra", brand: "Samsung", price: "1,39,999", image: "https://www.samsungmobilepress." },
  { id: "t3", name: "Dell Gaming laptop", brand: "dell", price: 99999, image: "https://i.dell.com/is/image/DellContent/content/dam/ss2/product-images/dell-client-products/notebooks/dell-plus/db16250/media-gallery/fpr/notebook-db16250nt-fpr-bl-gallery-2.psd?fmt=png-alpha&pscan=auto&scl=1&hei=804&wid=979&qlt=100,1&resMode=sharp2&size=979,804&chrss=full" },
  { id: "t4", name: "Nike AirMax", brand: "Nike", price: 119999, image: "https://static.nike.com/a/images/t_web_pdp_936_v2/f_auto/07bbe928-6af1-45ac-b1c6-9f66949ed933/NIKE+AIR+MAX+90+PRM.png" },
  { id: "t5", name: "Sony WH-1000XM5", brand: "Sony", price: 29999, image: "https://valueelectronics.com/wp-content/uploads/2022/05/WH-1000XM5_keyvisual_silver-Large-scaled.jpg" },
  { id: "t6", name: "Samsung Watch Ultra", brand: "Samsung", price: 52999, image: "https://images.samsung.com/is/image/samsung/p6pim/in/2407/gallery/in-galaxy-watch-ultra-l705-sm-l705fdaains-542169127?$684_547_PNG$" },
  { id: "t7", name: "Apple Watch Ultra", brand: "Apple", price: 11999, image: "https://media-ik.croma.com/prod/https://media.tatacroma.com/Croma%20Assets/Communication/Wearable%20Devices/Images/317431_0__5vYVlzqQN.png?updatedAt=1757600677496" },
  { id: "t8", name: "LOUIS VUITTON Jacket", brand: "LOUIS VUITTON", price: "3,49,999", image: "https://in.louisvuitton.com/images/is/image/lv/1/PP_VP_L/louis-vuitton-monogram-puffer-jacket--FUOW41MHI107_PM2_Front%20view.jpg" },
  { id: "t9", name: "LG Smart Refrigerator", brand: "LG", price: 65999, image: "https://www.lg.com/in/images/refrigerators/md07518083/gallery/GL-T412VPZX-Refrigerators-Front-View-DZ-01.jpg" },
  { id: "t10", name: "Philips 4K TV", brand: "Philips", price: 45999, image: "https://images.philips.com/is/image/PhilipsConsumer/50PUT8115_94-IMS-en_IN" },
  { id: "t11", name: "Raymond coat pant", brand: "Silk", price: "12,999", image: "https://www.thelibasstore.com/wp-content/uploads/2023/11/LSRAYTR341LGTSLVRGRYJAQSTRU140NOV-0.jpg" },
  { id: "t12", name: "Adidas Ultraboost 22", brand: "Adidas", price: 14999, image: "https://u-mercari-images.mercdn.net/photos/m28686690553_1.jpg" },
];

const mobiles = [
  { id: "m1", name: "iphone 17", brand: "Apple", price: "82,999", image: "https://www.apple.com/newsroom/images/2025/09/apple-debuts-iphone-17/geo/tile/Apple-iPhone-17-hero-250909-lp.jpg.news_app_ed.jpg" },
  { id: "m2", name: "Galaxy S25", brand: "Samsung", price: "80,999", image: "https://www.sammobile.com/wp-content/uploads/2025/01/Galaxy-s25-leaked-render-1.jpg" },
  { id: "m3", name: "Galaxy Fold 7", brand: "Samsung", price: "1,74,999", image: "https://images.samsung.com/is/image/samsung/assets/in/smartphones/galaxy-z-fold6/buy/Fold6_1600x864-4.jpg?imbypass=true" },
  { id: "m4", name: "Pixel 10 Pro Max", brand: "Google", price: "1,09,999", image: "https://vsprod.vijaysales.com/media/catalog/product/p/1/p10_pro_porcelain.jpeg?optimize=medium&fit=bounds&height=500&width=500" },
  { id: "m5", name: "iphone 16 Pro Max", brand: "Apple", price: "1,29,999", image: "https://www.dxomark.com/wp-content/uploads/drafts/post-178568/Apple-iPhone-16-Pro-Max_featured-image-packshot-review.png" },
  { id: "m6", name: "iPhone 16", brand: "Apple", price: "69,999", image: "https://vasanthandco.in/UploadedFiles/productimages/20240920014016-61DjMd76QnL._SX679_.jpg" },
  { id: "m7", name: "Galaxy S25+", brand: "Samsung", price: "99,999", image: "https://rukminim2.flixcart.com/image/480/640/xif0q/mobile/y/v/t/-original-imahggffvpcxr5jq.jpeg?q=90" },
  { id: "m8", name: "iphone 17 Pro", brand: "Apple", price: "1,34,999", image: "https://rukminim2.flixcart.com/image/480/640/xif0q/mobile/n/v/a/-original-imahft5nxmyqndhf.jpeg?q=90" },
  { id: "m9", name: "pixel 10", brand: "Google", price: "75,999", image: "https://rukminim2.flixcart.com/image/480/640/xif0q/mobile/t/j/e/-original-imahfjsfygfnyush.jpeg?q=90" },
  { id: "m10", name: "Galaxy S24 Ultra", brand: "Samsung", price: "89,999", image: "https://rukminim2.flixcart.com/image/480/640/xif0q/mobile/j/m/z/-original-imahgfmxumntk7sy.jpeg?q=90" },
   { id: "m11", name: "iPhone Air", brand: "Apple", price: "1,19,999", image: "https://m.media-amazon.com/images/I/614MsdnfBxL._AC_UF894,1000_QL80_.jpg" },
  { id: "m12", name: "Galaxy S25 Edge", brand: "Samsung", price: "1,01,999", image: "https://m.media-amazon.com/images/I/71Rn0YmeDuL.jpg" },
  { id: "m13", name: "OnePlus 15", brand: "OnePlus", price: "75,999", image: "https://media-ik.croma.com/prod/https://media.tatacroma.com/Croma%20Assets/Communication/Mobiles/Images/319445_0_Rq5I21hbG.png?updatedAt=1762164624521" },
  { id: "m14", name: "Vivo X 300 pro", brand: "Vivo", price: "89,999", image: "https://cdn1.smartprix.com/rx-iHNm0ZSqy-w1200-h1200/HNm0ZSqy.webp" },
  { id: "m15", name: "Galaxy Fold 6", brand: "Samsung", price: "1,10,999", image: "https://img-prd-pim.poorvika.com/product/samsung-galaxy-z-fold-6-5g-pink-256gb-12gb-ram-front-view.png" },
   { id: "m16", name: "iPhone 15", brand: "Apple", price: "49,999", image: "https://fdn2.gsmarena.com/vv/pics/apple/apple-iphone-15-1.jpg" },
  { id: "m17", name: "Galaxy FLIP 7", brand: "Samsung", price: "1,09,999", image: "https://images.samsung.com/is/image/samsung/assets/in/smartphones/galaxy-z-flip7/images/galaxy-z-flip7-features-gemini-live-start-mo.jpg?imbypass=true" },
  { id: "m18", name: "Galaxy Flip 6", brand: "OnePlus", price: "79,999", image: "https://images.samsung.com/in/smartphones/galaxy-z-flip6/images/galaxy-z-flip6-share-image.jpg" },
  { id: "m19", name: "Pixel 9", brand: "Google", price: "49,999", image: "https://media.tatacroma.com/Croma%20Assets/Communication/Mobiles/Images/309166_0_flwiwk.png" },
  { id: "m20", name: "Vivo X 200 pro", brand: "Vivo", price: "69,999", image: "https://i.gadgets360cdn.com/products/large/vivo-X200-pro-db-679x800-1728969363.jpg" },
  // ... add more for See All (20)
];

const laptops = [
  { id: "l1", name: "MacBook Pro M4 16 inch", brand: "Apple", price: "2,19,999", image: "https://inventstore.in/wp-content/uploads/2024/11/2-scaled.webp" },
  { id: "l2", name: "Samsung Book 5 Pro 16 inch", brand: "Samsung", price: "1,62,999", image: "https://images.samsung.com/is/image/samsung/p6pim/in/np960qha-kg3in/gallery/in-galaxy-book5-pro-360-16-inch-np960-538630-np960qha-kg3in-thumb-545246770" },
  { id: "l3", name: "Dell XPS 17", brand: "Dell", price: "2,19,999", image: "https://i.dell.com/is/image/DellContent/content/dam/ss2/product-images/dell-client-products/notebooks/xps-notebooks/xps-17-9730/spi/ng/notebook-xps-17-9730-silver-campaign-hero-504x350-ng.psd?fmt=jpg&wid=570&hei=400" },
  { id: "l4", name: "HP Envy 16", brand: "HP", price: "1,99,999", image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRO4vvRFZfcXvBAXgso2BfXaPFVJSWyN40umg&s" },
  { id: "l5", name: "Asus ROG Zephyrus", brand: "Asus", price: "1,19,999", image: "https://dlcdnwebimgs.asus.com/gain/92154016-46AB-4593-BE2B-588F86E55B4B/w717/h525" }
];

const appliances = [
  { id: "a1", name: "Samsung MICRO LED TV", brand: "Samsung", price: "1,14,99,999", image: "https://images.samsung.com/is/image/samsung/p6pim/in/mna110ms1acxxl/gallery/in-micro-led-2021-ms1a-mna110ms1acxxl-536972730?$684_547_PNG$" },
  { id: "a2", name: "Sony BRAVIA 8 II", brand: "Sony", price: "2,18,999", image: "https://media.tatacroma.com/Croma%20Assets/Entertainment/Television/Images/258523_0_lygda1.png" },
  { id: "a3", name: "LG 8kg Fully Automatic Front Load Washing Machine", brand: "LG", price: "45999", image: "https://www.lg.com/content/dam/channel/wcms/in/images/washing-machines/fhp1208z5m_abmqeil_eail_in_c/gallery/FHP1208Z5M-1-2010x1334.jpg/jcr:content/renditions/thum-1600x1062.jpeg" },
  { id: "a4", name: "Bosch Series 6 15 Place Settings Free Standing Dishwasher", brand: "Bosch", price: "65999", image: "https://media-ik.croma.com/prod/https://media.tatacroma.com/Croma%20Assets/Large%20Appliances/Dishwashers/Images/309171_0_tmfjcl.png?tr=w-600" },
  { id: "a5", name: "Samsung Refrigerator", brand: "Samsung", price: "89999", image: "https://images.samsung.com/is/image/samsung/p6pim/in/rs76cg80x0s9hl/gallery/in-side-by-side-smart-conversion-456566-rs76cg80x0s9hl-thumb-549051325" },
];

const clothes = [
  { id: "c1", name: "Fancy Women Dress", brand: "Fancy", price: "12,999", image: "https://assets.myntassets.com/dpr_1.5,q_30,w_400,c_limit,fl_progressive/assets/images/2024/OCTOBER/21/6yluxQL3_d01b23632c1c409ba30c65d6ecb9ca15.jpg" },
  { id: "c2", name: "Co Ord Set for Women Beach Dress", brand: "Co ord", price: 14999, image: "https://m.media-amazon.com/images/I/51736s8xRiL._AC_UY1100_.jpg" },
  { id: "c3", name: "Puma Classic Hoodie", brand: "Puma", price: "4,999", image: "https://assets.myntassets.com/w_412,q_30,dpr_3,fl_progressive,f_webp/assets/images/25072310/2023/10/27/cffff245-fd63-4e86-a2c4-fdb4f8efae501698386573267-Essential-Big-Logo-Regular-Fit-Mens-Hoodie-7821698386572914-1.jpg" },
  { id: "c4", name: "Levi’s Denim Jacket", brand: "Levi's", price: "7,999", image: "https://levi.in/cdn/shop/files/003F80004_02_Front.jpg?v=1736333972" },
  { id: "c5", name: "H&M Clothes", brand: "H&M", price: "1,999", image: "https://www2.hm.com/content/dam/ind-local-assets/1320713001_202502_LB_1762-2x3.jpg" },
];

const footwear = [
  { id: "f1", name: "Luxury Designer High Heels", brand: "Puma", price: "8,999", image: "https://image.made-in-china.com/2f0j00msZVyDrKlgzL/Brand-Heels-Luxury-Designer-High-Heels-Shoes-for-Women-Party-Wedding-Pumps-High-Quality-Dress-Shoes.webp" },
  { id: "f2", name: "Red Chief Men's Leather Casual Shoes", brand: "Red Chief", price: "9,999", image: "https://m.media-amazon.com/images/I/61G8T8c8yVL._AC_UY1000_.jpg" },
  { id: "f3", name: "Nike Air Force 1", brand: "Nike", price: "10,999", image: "https://static.nike.com/a/images/t_web_pw_592_v2/f_auto/36a5ba42-47b8-44c3-81f4-d491786575e5/W+NIKE+VOMERO+PREMIUM.png" },
  { id: "f4", name: "JM LOOKS Fashion Fancy Stylish Casual Heel Sandal", brand: "Skechers", price: "7,999", image: "https://m.media-amazon.com/images/I/51s1Px2ekOL._AC_UY1000_.jpg" },
  { id: "f5", name: "Bata Formal Leather", brand: "Bata", price: "5,999", image: "https://www.bata.com/dw/image/v2/BCLG_PRD/on/demandware.static/-/Sites-bata-in-master-catalog/default/dwb08c371e/images/large/8246837_8.jpeg?sw=620%20620w" },
];

// Best deals (4 + 4 for 50% off)
// ✅ Corrected Best Deals Section
const bestDeals = [
  {
    id: "d1",
    name: "Sony 4K Smart TV",
    brand: "Sony",
    price: "1,44,999",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQWP3lP47hcgio3sBDpFqIpwi0cOvRG5U67-A&s",
    discount: 50,
  },
  {
    id: "d2",
    name: "Nike Air Max 270",
    brand: "Nike",
    price: "12,999",
    image: "https://static.nike.com/a/images/t_web_pdp_936_v2/f_auto,u_126ab356-44d8-4a06-89b4-fcdcc8df0245,c_scale,fl_relative,w_1.0,h_1.0,fl_layer_apply/91aefeeb-0efb-4387-b516-5fc4d9ed5709/WMNS+AIR+JORDAN+1+LOW.png",
    discount: 40,
  },
  {
    id: "d3",
    name: "MacBook Air M4",
    brand: "Apple",
    price: "99,999",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRMZZZR0iR3p_2ZQcb-8VxLIw8f0sGRoF6GLw&s",
    discount: 30,
  },
  {
    id: "d4",
    name: "Philips Air Fryer",
    brand: "Philips",
    price: "10,999",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQSm9J0Gw5IN7KlKSV7mB0DQLxhXYU-3-tzkw&s",
    discount: 35,
  },
  {
    id: "s1",
    name: "Festive Saree",
    brand: "Zari",
    price: "14,999",
    image: "https://static.wixstatic.com/media/faf1ba_8e61a7e398744cc7bf39deab4da8c0a3~mv2.jpg/v1/fill/w_526,h_691,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/faf1ba_8e61a7e398744cc7bf39deab4da8c0a3~mv2.jpg",
    discount: 50,
  },
  {
    id: "s2",
    name: "Diwali Lights Set",
    brand: "FestiveCo",
    price: "799",
    image: "https://m.media-amazon.com/images/I/515jA106Y7L._AC_UF1000,1000_QL80_.jpg",
    discount: 60,
  },
  {
    id: "s3",
    name: "Leather Jacket",
    brand: "Zara",
    price: "3,999",
    image: "https://assets.myntassets.com/dpr_1.5,q_30,w_400,c_limit,fl_progressive/assets/images/2025/SEPTEMBER/15/y7pXz9d1_6fcb9d865468416b99e2ce0b66aeef5a.jpg",
    discount: 45,
  },
  {
    id: "s4",
    name: "Samsung Refrigerator",
    brand: "Samsung",
    price: "3,60,999",
    image: "https://www.sammobile.com/wp-content/uploads/2024/05/Samsung-2024-Bespoke-AI-Family-Hub-refrigerator.jpg",
    discount: 50,
  },
  {
    id: "s5",
    name: "Sabyasachi Saree",
    brand: "Sabyasachi",
    price: "8,499",
    image: "https://assets.vogue.in/photos/5ce423e17a66d22801e762d7/2:3/w_2560%2Cc_limit/Sabyasachi-ss18-featured.jpg",
    discount: 50,
  },
  {
    id: "s6",
    name: "Ladies Stylish Lehenga Choli with Dupatta",
    brand: "Traditional Wear",
    price: "21,499",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQmXT3-REEfUb6FG_pdScxPdMudL7tVsyR9g&s",
    discount: 50,
  },
];


// More products (short sample here — expand to 200 in actual file)
const moreProducts = [
  // add 200 curated items here; example few:
  { id: "mp1", name: "Sony Alpha 7 IV Full-Frame Mirrorless Interchangeable Lens Camera with 28-70mm Zoom Lens Kit", brand: "Sony", price: "4,99,999", image: "https://shopatsc.com/cdn/shop/products/CX88500_SEL2470GM_right.jpg?v=1697633535" },
  { id: "mp2", name: "SONY PlayStation5 Console (slim) CFI-2008A01X 1 TB", brand: "SONY", price: "60,000", image: "https://www.designinfo.in/wp-content/uploads/nc/p/5/1/9/2/3/51923-485x485.jpg" },
  { id: "mp3", name: "LG 20 L Solo Microwave Oven MS2043BP", brand: "LG", price: "10,999", image: "https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcS26tMR3vjplvmwQfkXBOuoVetpWiCbvQP4HCO7R8GrzdzkQDJDe6bxUyRfFGUF_4wakJ3-LFwmEH1djlSZyIftUkdIedEh" },
  { id: "mp4", name: "Logitech MX Keys S Wireless Keyboard", brand: "Logitech", price: "11,499", image: "https://rukmini1.flixcart.com/image/1500/1500/xif0q/keyboard/p/e/k/-original-imahbg3nvkraxpan.jpeg?q=70" },
  { id: "mp5", name: "Logitech MX Master 3", brand: "Logitech", price: "22,999", image: "https://m.media-amazon.com/images/I/61xKiCADfpL.jpg" },
  // ... continue up to 200
];

// Export all
const homepageData = {
  heroBanners,
  trending,
  mobiles,
  laptops,
  appliances,
  clothes,
  footwear,
  bestDeals,
  moreProducts,
};

export default homepageData;
